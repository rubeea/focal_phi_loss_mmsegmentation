...
def phi_loss(inputs, targets, smooth, weight=None, reduction='mean',
                  avg_factor=None):
    ...
    phi = ((TP * TN - FP * FN) + smooth) / ((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN)) 
    loss= (1. - phi) #phi_loss
...
class PhiLoss(nn.Module):
    ...
    def forward(self,
        ...
        phi_loss_val= phi_loss(inputs, label, self.smooth,
